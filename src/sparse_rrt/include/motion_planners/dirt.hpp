...
#ifndef SPARSE_PLANNER_DIRT_HPP
#define SPARSE_PLANNER_DIRT_HPP
 ...
 */
class dirt_node_t : public tree_node_t
{
public:
	dirt_node_t() : tree_node_t()
	{
		inactive = false;
		dir_radius = 0;
		h=0;
	...
	std::vector<std::tuple<double*,double,double*>> maneuvers;
	std::vector<int> man_index;
...
/**
 * @brief The motion planning algorithm DIRT (Dominance Informed Region Tree)
 * @details The motion planning algorithm DIRT (Dominance Informed Region Trees)
 */
class dirt_t : public planner_t
{
public:
	/**
	 * @copydoc planner_t::planner_t(system_t*)
	 */
	dirt_t(system_t* in_system) : planner_t(in_system)
	{
...
protected:
	...
	bool child_selection;

	/**
	 * @brief The best goal node found so far.
	 */
	dirt_node_t* best_goal;
